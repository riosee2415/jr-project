...
<div class="subpage">
  <div class="sub-container" id="sub-container-js">
    <div class="sub-content-wrap">
      ...
      <form action="/news.detail.do" method="get" id="frm-${fn:toLowerCase(b_type_1)}-detail">
			...
			<form action="/news.write.do" method="get" id="frm-${fn:toLowerCase(b_type_1)}-write">
				<input type="hidden" name="parent" value="${param.parent }" />
				<input type="hidden" name="code" value="${param.code }" /> 
				<input type="hidden" name="tab" value="1" />
				<input type="hidden" name="s_type" /> 
    ...
    <div class="sub-content-wrap">
      ...
      <form action="/news.detail.do" method="get" id="frm-${fn:toLowerCase(b_type_2)}-detail">
		    <input type="hidden" name="parent" value="${param.parent }" />
		    <input type="hidden" name="code" value="${param.code }" />
			...
			<form action="/news.write.do" method="get" id="frm-${fn:toLowerCase(b_type_2)}-write">
				<input type="hidden" name="parent" value="${param.parent }" />
				<input type="hidden" name="code" value="${param.code }" /> 
				<input type="hidden" name="tab" value="2" />
				<input type="hidden" name="s_type" /> 
    ...
    <div class="sub-content-wrap">
      ...
      >
        ...
        <div class="board-pc-ver">
          <table class="board-list__table">
            ...
            <tbody id="${fn:toLowerCase(b_type_3)}-board-js"></tbody>
          </table>
        </div>
        ...
        <div class="board-mobile-ver">
          <div
            class="board-mobile-wrap"
            id="${fn:toLowerCase(b_type_3)}-board-m-js"
          ></div>
        </div>

        <div id="${fn:toLowerCase(b_type_3)}-paging-js"></div>

        <div class="board-btn__box" id="${fn:toLowerCase(b_type_3)}-button-js">
          <c:if test="${sessionScope.loginRight <= AcceptRight_3.WRITE_RIGHT}">
            <button
              type="button"
              onclick="javascript:boardWriteMoveHandler('${b_type_3}', '${searchType}', '${searchKeyword}')"
              >글쓰기</button
            >
          </c:if>
        </div>
      ...
      <form action="/news.detail.do" method="get" id="frm-${fn:toLowerCase(b_type_3)}-detail">
		    <input type="hidden" name="parent" value="${param.parent }" />
		    <input type="hidden" name="code" value="${param.code }" />
			...
			<form action="/news.write.do" method="get" id="frm-${fn:toLowerCase(b_type_3)}-write">
				<input type="hidden" name="parent" value="${param.parent }" />
				<input type="hidden" name="code" value="${param.code }" /> 
				<input type="hidden" name="tab" value="3" />
				<input type="hidden" name="s_type" /> 
