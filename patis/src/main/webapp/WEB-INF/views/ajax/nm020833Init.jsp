...
<c:choose>
	...
	<c:otherwise>
		<c:forEach var="board" items="${boardList }">
			<c:choose>
				<c:when test="${board.b_NOTICE eq 1 }">
					<div class="border-lsit_wrap" onclick="javascript:boardDetailMoveHandler('${board.b_TYPE}', '${board.b_NO}', '${board.ROWNUM}')">
		        <table class="border-list__table2">
		          <thead>
		            <tr class="row-notice">
		              <th>공지</th>
		          ...
		          <tbody>
		            <tr>
		              <td colspan="2"><c:out value="${board.b_AUTHOR_NO eq 0 ? board.b_AUTHOR_NONE : board.b_AUTHOR}" /></td>
		              <td>${board.b_CREATE_TIME }</td>
		              <td>${board.b_HIT }</td>
		            </tr>
		          </tbody>
